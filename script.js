...
/* =========================================
   3. التهيئة (بدون دخول أوتوماتيك)
   ========================================= */
document.addEventListener('DOMContentLoaded', () => {
    // مراقب حالة الدخول (بس مش بيدخلك اللعبة علطول)
    auth.onAuthStateChanged(async user => {
        if(user) {
            state.me = user.uid;
            // لو مسجل دخول، هات بياناته بس متدخلوش اللوبي، خليه في شاشة الانتظار
...
async function registerUser() {
    ...
    // 🛑 1. التحقق من عدم تكرار الاسم في الداتا بيز
    const nameCheck = await db.collection('users').where('name', '==', name).get();
    if(!nameCheck.empty) {
        return toast('هذا الاسم مستخدم بالفعل! اختر اسماً آخر 🚫', true);
    }
    ...
    try {
        ...
        console.error(e);
        if(e.code === 'auth/email-already-in-use') toast('البريد مسجل بالفعل', true);
        else toast('خطأ في التسجيل', true);
    }
}

async function loadUserProfile(uid) {
    // تحميل البيانات وعرض اللوبي
    try {
        ...
        if(doc.exists) {
            ...
            // ✅ هنا بس بندخل اللوبي (بعد التأكد من التحميل)
            enterGlobalLobby();
        }
...
async function logoutUser() {
    // قبل الخروج، نحذف اللاعب من اللوبي عشان ميفضلش معلق
    if(state.me) {
...
/* =========================================
   5. اللوبي (مع منع التكرار)
   ========================================= */
async function enterGlobalLobby() {
    // استخدام set مع uid بيضمن ان اللاعب ميتكررش، لو موجود هيتعمل له update
    ...
    await playerRef.set({
        ...
        uid: state.me,
        scores: [], // تصفير السكور عند الدخول الجديد
        status: 'waiting', 
        lastSeen: firebase.firestore.FieldValue.serverTimestamp()
    }, { merge: true }); // merge مهم عشان منحذفش حقول تانية لو وجدت
...
function subscribe() {
    ...
    unsubPlayers = db.collection('rooms').doc(GAME_ID).collection('players').onSnapshot(snap => {
        state.players = [];
        // هنا بنفرغ المصفوفة وبنملاها من جديد عشان نضمن عدم التكرار البصري
...
// ... (باقي الدوال togglePlayerStatus, startGame, resetGame زي ما هي) ...
...
// ... (باقي الدوال المساعدة كما هي تماماً بدون تغيير) ...
...
async function finishGameAndSave() { if(!confirm('حفظ النتيجة في السجل وإنهاء اللعبة؟')) return; const active = state.players.filter(p => p.status === 'active'); if(active.length < 2) return toast('لا يوجد لاعبين', true); const sorted = active.sort((a,b) => { const sa = (a.scores||[]).reduce((x,y)=>x+(Number(y)||0),0); const sb = (b.scores||[]).reduce((x,y)=>x+(Number(y)||0),0); return sa - sb; }); const gameData = { date: firebase.firestore.FieldValue.serverTimestamp(), owner: state.me, lionName: sorted[0].name, lionScore: (sorted[0].scores||[]).reduce((x,y)=>x+(Number(y)||0),0), sheepName: sorted[sorted.length-1].name, sheepScore: (sorted[sorted.length-1].scores||[]).reduce((x,y)=>x+(Number(y)||0),0), playersCount: active.length }; try { await db.collection('history').add(gameData); playSound('winAudio'); toast('تم الحفظ 🏆'); await db.collection('rooms').doc(GAME_ID).update({ status: 'lobby', round: 1 }); const batch = db.batch(); state.players.forEach(p => batch.update(db.collection('rooms').doc(GAME_ID).collection('players').doc(p.id), { scores: [], status: 'waiting' })); await batch.commit(); } catch(e) { console.error(e); } }
async function showHallOfFame() { const list = document.getElementById('fameList'); list.innerHTML = 'جاري التحميل...'; document.getElementById('fameModal').style.display = 'flex'; try { const snap = await db.collection('history').orderBy('date', 'desc').limit(20).get(); list.innerHTML = ''; if(snap.empty) list.innerHTML = '<div style="text-align:center">لا يوجد سجلات</div>'; snap.forEach(d => { const r = d.data(); const date = r.date ? r.date.toDate().toLocaleDateString('ar-EG') : ''; const el = document.createElement('div'); el.className='fame-item'; el.innerHTML = `<div class="fame-date">${date}</div><div class="fame-row"><span class="lion-badge">🦁 ${r.lionName}</span> <span class="score-badge">${r.lionScore}</span></div><div class="fame-row"><span class="sheep-badge">🐑 ${r.sheepName}</span> <span class="score-badge">${r.sheepScore}</span></div>`; list.appendChild(el); }); } catch(e) { list.innerHTML = 'خطأ في التحميل (تأكد من الفهرسة)'; } }
...
function openAdminSelect() { const list = document.getElementById('adminCandidatesList'); list.innerHTML = ''; state.players.forEach(p => { if(p.uid === state.me) return; const item = document.createElement('div'); item.className = 'lobby-item'; item.textContent = p.name; item.onclick = () => transferAdmin(p); list.appendChild(item); }); document.getElementById('adminSelectModal').style.display = 'flex'; }
